...
function ensureOwners(db: ReturnType<typeof getJsonDb>) {
  ...
  const units = db.units as { id: number; projectId: number }[] | undefined;
  if (backfillOwnerProjectIds(db.owners as OwnerRecord[], units)) {
    saveJsonDb(db);
  }
}
...
function listForUser(db: ReturnType<typeof getJsonDb>, projectId: number | null | undefined) {
  ...
  return filterOwnersForProject(db.owners as OwnerRecord[], projectId, units);
}
...
function assertOwnerAccess(
  ...
  if (!o) return null;
  const units = db.units as { id: number; projectId: number }[];
  if (!ownerBelongsToProject(o, projectId, units)) return null;
  return o;
}
