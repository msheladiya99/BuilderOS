...
async function seed() {
  ...
  try {
    ...
    // Demo project + units in tenant schema
    ...
    if (!projectId) {
      const existing = await client.query(`SELECT id FROM ${schema}.projects LIMIT 1`);
    ...
    console.log("  Users:       arjun@, sales@, accounts@, site@builderos.in / password");
  } catch (err) {
